...
export class  AuthDbInterface {
  ...
  getAll():Promise<AuthEntity[] | null>{
    return this.authRepository.createQueryBuilder('getAllUsername')
    .select([
      'logIn.username',
    ])
    .where(
  ...
  async checkUserExists(username:string, email:string){
    ...
    const user:AuthEntity | null = await this.authRepository.findOne({where: [
      {username},
      {email}
    ]})

    if(user)
      throw new ConflictException('username or email does exists.')
  ...
  private async checkUniquePrometers ( data ){
    ...
    try {
      ...
      await this.authRepository.createQueryBuilder('check')
        .select([
          'check.username',
        ])
        .where(
