...
export class TaskController {
    ...
    @UseGuards(AuthGuard)
    @Patch('/item:itemId')
    async updateItem(@Request() req: Req, @Body() dto: UpdateItemDto, @Param('itemId') itemId:string){
        const username = req['user'].username;
        try{
            await this.taskService.isUserModifyAccess(username, itemId);
        }catch{
            throw new ForbiddenException('resource not found or you can not chnage it.')
        }
    ...
    @UseGuards(AuthGuard)
    @Post('/report:itemId')
    async addReport(@Request() req: Req,  @Body() dto: AddReportDto, @Param('itemId') itemId:string){
        const username = req['user'].username;
        try{
            await this.taskService.isUserModifyAccess(username, itemId);
        }catch{
    ...
    @UseGuards(AuthGuard)
    @Put('/report:itemId')
    async updateReport(@Request() req: Req,  @Body() dto: UpdateReportDto, @Param('itemId') itemId:string){
        const username = req['user'].username;
        try{
            await this.taskService.isUserModifyAccess(username, itemId);
        }catch{
